...
export default function CreateUpdateGroupModal({
  ...
  const {
    ...
    defaultValues: {
      ...
      teacherId: lectors[0]?.id.toString() || "",
    },
  });
  ...
  useEffect(() => {
    if (data && isOpen && type === "update") {
      reset({
        name: data.name || "",
        courseId: data.Courses?.at(0)?.id.toString(),
        teacherId: data.Courses?.at(0)?.teacherId?.toString() || "",
      });
    }
  }, [data, isOpen, reset]);
  ...
  useEffect(() => {
    ...
    if (selectedCourse && selectedCourse.teacherId) {
      const teacher = lectors.find(
        (lector) => lector.id === selectedCourse.teacherId,
      );
      if (teacher?.id !== parseInt(teacherId)) {
        setMessage(
          `Tento kurz aktuálně vyučuje ${teacher?.firstName} ${teacher?.lastName}. Chcete přepsat lektora?`,
        );
      } else {
        setMessage("");
      }
    }
  }, [courseId, courses, lectors, teacherId]);
